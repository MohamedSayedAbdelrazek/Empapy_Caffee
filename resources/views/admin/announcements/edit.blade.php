                ...
                <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('message_ar') is-invalid @enderror" id="message_ar" name="message_ar"
                            rows="3" required>{{ old('message_ar', $announcement->message_ar) }}</textarea>
                        @error('message_ar')
                    ...
                    <!-- Hidden icon field -->
                    <input type="hidden" name="icon" value="{{ old('icon', $announcement->icon) }}">
                        ...
                        <div class="col-md-12">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('order') is-invalid @enderror"
                                    id="order" name="order" value="{{ old('order', $announcement->order) }}"
                                    min="0" required>
                                @error('order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save me-2"></i>تحديث الإعلان
                        </button>
